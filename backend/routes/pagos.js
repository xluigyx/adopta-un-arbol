...
router.post("/", upload.single("comprobante"), async (req, res) => {
  try {
    const { userId, nombreUsuario, paqueteId, paqueteNombre, creditos, bonus, precio, notas } = req.body;
...
// 🔔 GET /api/pago/pending — contar pagos pendientes
...
// ✅ PUT /api/pago/:id/estado — actualizar estado (Aprobado o Rechazado)
router.put("/:id/estado", async (req, res) => {
  try {
    ...
    if (estado === "Aprobado") {
      ...
      if (usuario) {
        usuario.puntostotales += pago.paquete.creditos + (pago.paquete.bonus || 0);
        await usuario.save();
      }
    }

    res.json({ success: true, message: `Pago ${estado.toLowerCase()} correctamente` });
  } catch (error) {
